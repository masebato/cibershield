...
module.exports.profileUpdate = async (req, res) => {
  ...
  const profile = await authService.updateProfile(req.user.sub, { company_name, sector });
  res.json(profile);
};

module.exports.assetsCreate = async (req, res) => {
  const { type, value } = req.body;
  const asset = await AssetModel.create({ company_id: req.user.company_id, type, value });
  res.json({
    id:         asset.id,
    type:       asset.type,
    value:      asset.value,
...
module.exports.assetsDelete = async (req, res) => {
  ...
  if (!deleted) {
    return res.status(400).json({ error: 'Asset not found or not owned by company' });
  }
